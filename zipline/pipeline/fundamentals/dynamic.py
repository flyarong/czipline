...
def normalized_dividend_data(df):
    ...
    raw_df['asof_date'] = pd.to_datetime(
        raw_df['asof_date']) - pd.Timedelta(days=1)
    return raw_df
...
def _save_short_name():
    """存储股票简称变动历史"""
    with session_scope() as sess:
        query = sess.query(StockDaily.code, StockDaily.date,
                           StockDaily.A001_名称).group_by(
                               StockDaily.code, StockDaily.A001_名称)
        df = pd.DataFrame.from_records(query.all())
        df.columns = ['sid', 'asof_date', 'short_name']
        df['asof_date'] = pd.to_datetime(df['asof_date'], utc=True)
        df['timestamp'] = df['asof_date']  # - pd.Timedelta(days=1)
        df['sid'] = df['sid'].map(lambda x: int(x))
    table_name = 'short_names'
    # 转换为bcolz格式并存储
